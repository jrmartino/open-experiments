...
public abstract class AbstractAuthorizablePostServlet extends SlingAllMethodsServlet {
  ...
  protected void doPost(SlingHttpServletRequest request,
      SlingHttpServletResponse httpResponse) throws ServletException, IOException {
    ...
    try {
      ...
      for (Modification change : changes) {
        ...
        case DELETE:
          ...
          break;
        case MOVE:
          htmlResponse.onMoved(change.getSource(), change.getDestination());
          break;
        case COPY:
          htmlResponse.onCopied(change.getSource(), change.getDestination());
          break;
        case CREATE:
          htmlResponse.onCreated(change.getSource());
          break;
        case ORDER:
          htmlResponse.onChange("ordered", change.getSource(), change.getDestination());
          break;
        }
      }
  ...
  protected Map<String, RequestProperty> collectContent(SlingHttpServletRequest request,
      HtmlResponse response, String authorizablePath) {
  ...
  private RequestProperty getOrCreateRequestProperty(Map<String, RequestProperty> props,
      String paramPath, String suffix) {
  ...
  /**
   * Removes all properties listed as {@link RequestProperty#isDelete()} from the
   * authorizable.
   * 
   * @param authorizable
   *          The <code>org.apache.jackrabbit.api.security.user.Authorizable</code> that
   *          should have properties deleted.
   * @param reqProperties
   *          The map of request properties to check for properties to be removed.
   * @param response
    ...
    for (RequestProperty prop : reqProperties.values()) {
      if (prop.hasValues()) {
        ...
        if (authorizable instanceof Group) {
          ...
          if (prop.getName().equals("userId") || prop.getName().equals("pwd")
  ...
  /**
   * Returns an external form of the given path prepending the context path and appending
   * a display extension.
   * 
   * @param path
  ...
  /**
   * Returns <code>true</code> if the <code>name</code> starts with either of the prefixes
   ...
   * {@link SlingPostConstants#ITEM_PREFIX_ABSOLUTE <code>/</code>}.
   */
   ...
   * {@link SlingPostConstants#ITEM_PREFIX_RELATIVE_CURRENT <code>./</code>}. In this case
   * only parameters starting with either of the prefixes
   * {@link SlingPostConstants#ITEM_PREFIX_RELATIVE_CURRENT <code>./</code>},
   * {@link SlingPostConstants#ITEM_PREFIX_RELATIVE_PARENT <code>../</code>} and
   * {@link SlingPostConstants#ITEM_PREFIX_ABSOLUTE <code>/</code>} are considered as
   * providing content to be stored. Otherwise all parameters not starting with the
   * command prefix <code>:</code> are considered as parameters to be stored.
   */
